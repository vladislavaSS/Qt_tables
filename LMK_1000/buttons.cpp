...
void LMK1000Widget::hasChildren (QStandardItem *selectedItem, rapidjson::Document::AllocatorType &allocator, rapidjson::Value &jsonArray, QStandardItemModel* model, QTreeView* treeView) {
            ...
            if (item && item->hasChildren()) {
                ...
                for (const auto& value : jsonArray.GetArray()) {
                    ...
                    if (value.HasMember("Data") && value["Data"].IsString()) {
                        dataStrings.append(QString::fromStdString(value["Data"].GetString()));
                    }
                }

                for (int index = 0; index < qMin(addresses.size(), dataStrings.size()); ++index) {
                    if ((item->text() == addresses[index] && LMKflag) || !LMKflag) updateChildWidgets(model, dataStrings[index], item, addresses[index]);
                }
            }
        }

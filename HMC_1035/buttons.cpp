...
void HMC1035Widget::onParentItemClicked(const QModelIndex& index) {
...
void HMC1035Widget::saveItem(QStandardItem *selectedItem) {

    if (selectedItem->parent() != nullptr) {
        if (selectedItem->parent()->text() == "PLL" || selectedItem->parent()->text() == "VCO Subsystem") HMCflag = 1;
        else {
    ...
    if (HMCflag) hasChildren(selectedItem->parent(), allocator, jsonArray, model, treeView);
...
void HMC1035Widget::on_save_button_clicked()
{
    rapidjson::Document document;
    document.SetObject();
    rapidjson::Document::AllocatorType& allocator = document.GetAllocator();
...
void HMC1035Widget::updateChildWidgets(QStandardItemModel* model, QString dataString, QStandardItem* item, QString readItem) {
...
void HMC1035Widget::load_all() {
    load("output.json");
}

void HMC1035Widget:: load_element() {
    load("save_element.json");
}

void HMC1035Widget::load(const QString& filePath)
{
    QStandardItemModel* model = qobject_cast<QStandardItemModel*>(treeView -> model());
